...
class LogoUploadHandler {
    ...
    protected $error_messages = array(
        ...
        'validation_failed' => 'File was not validated correctly due to previous errors.',
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . C::D('LOGO_PATH'),
            'upload_url' => $this->get_full_url() . C::D('LOGO_PATH'),
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'max_width' => C::D('LOGO_IMAGE_MAX_WIDTH'),
            'max_height' => C::D('LOGO_IMAGE_MAX_HEIGHT'),
            'min_width' => C::D('LOGO_IMAGE_MIN_HEIGHT'),
            'min_height' => C::D('LOGO_IMAGE_MIN_HEIGHT'),
    ...
    protected function post_update_db($files) {
        // Only files that were uploaded without errors should be listed in DB.
        /*if (is_array($files) && isset($files[0])){
            $items_model = $this->options['items_model'];
            $items_model->updateItemLogo($this->get_item_id(), $files[0]);
        }*/
        
        F::setPendingUploadedFile('logo/update', $this->get_item_id(), $files[0]->name);
        
    }

    protected function delete_update_db($name) {
        /*
        $items_model = $this->options['items_model'];
        $items_model->updateItemLogo($this->get_item_id(), 0);
        */
        
        F::setPendingUploadedFile('logo/delete', $this->get_item_id(), $files[0]->name);        
    }
    ...
    protected function get_user_path() {
        if ($this->options['user_dirs']) {
            return str_replace("//", "/", $this->get_item_id());
        }
        return '';
    }
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        return str_replace("//", "/",$this->options['upload_dir'] . $this->get_user_path()
    ...
    protected function get_upload_path_logo($filename){
        $oldPath = $this->get_upload_path($filename);
        $oldPathInfo = pathinfo($oldPath);
        
        $newPath = $oldPathInfo['dirname'].'/'.$this->get_item_id().'.'.$oldPathInfo['extension'];
        $newName = $this->get_item_id().'.'.$oldPathInfo['extension'];
        return array('path'=>$newPath, 'file'=>$newName);
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {       
            ...
            $newFileInfo = $this->get_upload_path_logo($file->name);
            $file_path = $newFileInfo['path'];
            $file->name = $newFileInfo['file']; 
            ...
            // If no error is set here then eventhough validation failed, script continued its processing and updated DB
            //$file->error = $file->error.' '.$this->get_error_message('validation_failed');
        }
        return $file;
    }
    ...
    public function delete($print_response = true) {
        ...
        foreach ($file_names as $file_name) {
            ...
            if ($success) {
                ...
                $this->delete_update_db($this->options['item_name']);
            }
